...
| Combo Last 4 | `Combo_Last4__c` | Text(4), encrypted | NEVER store full combo |
| Key Holders Count | `Key_Holders_Count__c` | Number | |
| Backup Key Location | `Backup_Key_Location__c` | Text Area (255) | |
| Photo URL | `Photo_URL__c` | URL | Supabase storage URL |
| Verified | `Verified__c` | Checkbox | Driver verifies on first pickup |
...
| Field | API Name | Type | Notes |
|---|---|---|---|
| Bank Account Last 4 | `Bank_Account_Last4__c` | Text(4) | Display-only |
...
2. `Combo_Last4__c` — is the existing platform encryption license sufficient, or do we need Shield?
