...
export async function sendOpsHandoffEmail(
  ...
  // 2) Create the EmailMessage record.
  // Status = '3' = Sent. MessageDate stamps the activity time.
  ...
  // 3) Link the PDF to the EmailMessage so it shows as an attachment in the
  //    activity timeline. ShareType='V' = Viewer (inferred members), Visibility='I'.
  ...
  //    but accessible from REST. If your org doesn't have it enabled, the
  //    EmailMessage record above is still created (logged but not delivered).
  //    We swallow errors here so the queue records the email-as-logged
  //    even if delivery fails — ops can manually re-send from SF.
  try {
      ...
      inputs: [
        {
          ...
          senderAddress: params.orgWideEmailAddressId,
    ...
    // Log and continue — the EmailMessage record is already on the Opp.
    console.error(
      '[send-ops-handoff] emailSimple delivery failed (record still logged):',
